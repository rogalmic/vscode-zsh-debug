...
import { validatePath } from './zshRuntime';
import { getWSLPath, reverseWSLPath, escapeCharactersInZshdbArg, getWSLLauncherPath } from './handlePath';
import { EventSource } from './eventSource';
import { spawnZshScript } from './spawnZsh';
...
export class ZshDebugSession extends LoggingDebugSession {
	...
	protected disconnectRequest(response: DebugProtocol.DisconnectResponse, _args: DebugProtocol.DisconnectArguments): void {
		...
		spawnZshScript(`${this.launchArgs.pathPkill} -KILL -P "${this.debuggerProcessParentId}"; ${this.launchArgs.pathPkill} -TERM -P "${this.proxyData["PROXYID"]}"`,
	...
	protected async launchRequest(response: DebugProtocol.LaunchResponse, args: LaunchRequestArguments): Promise<void> {
		...
		const errorMessage = validatePath(
			args.cwdEffective, args.pathZsh, args.pathZshdb, args.pathCat, args.pathMkfifo, args.pathPkill);
		...
		this.proxyProcess = spawnZshScript(
		`function cleanup()
		{
			exit_code=$?
		...
		if (this.launchArgs.terminalKind === "debugConsole" || this.launchArgs.terminalKind === undefined) {
			...
			const termArgs: DebugProtocol.RunInTerminalRequestArguments = {
				...
				cwd: ".",
				args: [currentShell, optionalZshPathArgument, `-c`, command].filter(arg => arg !== ""),
			};
	...
	protected async setBreakPointsRequest(response: DebugProtocol.SetBreakpointsResponse, args: DebugProtocol.SetBreakpointsArguments): Promise<void> {
		...
		if (sourcePath !== undefined) {
			sourcePath = escapeCharactersInZshdbArg(sourcePath);
		}
	...
	protected async threadsRequest(response: DebugProtocol.ThreadsResponse): Promise<void> {
		...
		response.body = { threads: [new Thread(ZshDebugSession.THREAD_ID, "Zsh thread")] };
		this.sendResponse(response);
	}
			...
			if (this.promptReached(currentOutputLength)) {
				...
				for (let i = currentOutputLength; i <= lastStackLineIndex; i++) {
					...
					frames.push(new StackFrame(
						frameIndex,
						frameText,
						fs.existsSync(frameSourcePath) ? new Source(basename(frameSourcePath), this.convertDebuggerPathToClient(frameSourcePath), undefined, undefined, 'zsh-adapter-data') : undefined,
	...
	// zshdb doesn't support reverse execution
	...
	protected async evaluateRequest(response: DebugProtocol.EvaluateResponse, args: DebugProtocol.EvaluateArguments): Promise<void> {
		...
		expression = escapeCharactersInZshdbArg(expression);
	...
	protected async pauseRequest(response: DebugProtocol.PauseResponse, args: DebugProtocol.PauseArguments): Promise<void> {
		if (args.threadId === ZshDebugSession.THREAD_ID) {
			spawnZshScript(`${this.launchArgs.pathPkill} -INT -P ${this.debuggerProcessParentId} -f zshdb`,
				this.launchArgs.pathZsh,
				data => this.sendEvent(new OutputEvent(`${data}`, 'console')))
		...
		this.outputEventSource.schedule(() => {
			for (; this.fullDebugOutputIndex < this.fullDebugOutput.length - 1; this.fullDebugOutputIndex++) {
				...
				if (line.indexOf("(") === 0 && line.indexOf("):") === line.length - 2) {
					...
					this.sendEvent(new StoppedEvent("break", ZshDebugSession.THREAD_ID));
				}
...
DebugSession.run(ZshDebugSession);
